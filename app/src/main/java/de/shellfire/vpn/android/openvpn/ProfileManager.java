...
public class ProfileManager {
    ...
    private static VpnProfile get(String key) {
        ...
        VpnProfile result = instance.profiles.get(key);
        Log.d("ProfileManager", "get(" + key + ") - result " + result);
        return result;
    ...
    private static void saveProfile(Context context, VpnProfile profile, boolean updateVersion, boolean isTemporary) {
        ...
        try {
            ...
            vpnFile = new ObjectOutputStream(fos);
            Log.d("ProfileManager", "saveProfile() - saving to " + context.getFilesDir() + "/" + filename);
    ...
    public static VpnProfile get(Context context, String profileUUID, int version, int tries) {
        VpnStatus.logError("ProfileManager.get() - trying to get profileUUID " + profileUUID + " tries " + tries);
        checkInstance(context);
        VpnProfile profile = get(profileUUID);
        int tried = 0;
        while ((profile == null || profile.mVersion < version) && (tried++ < tries)) {
            Log.d("ProfileManager", "get() - profile is null ? " + (profile == null ? "yes" : "no") + " tried " + tried + " times");
            try {
                Thread.sleep(100);
        ...
        if (tried > 5) {
            int ver = profile == null ? -1 : profile.mVersion;
            VpnStatus.logError(String.format(Locale.US, "Used x %d tries to get current version (%d/%d) of the profile %s", tried, ver, version, profileUUID));
            VpnStatus.logError("ProfileManager.get() - profile is null ? " + (profile == null ? "yes" : "no"));
        }
        Log.d("ProfileManager", "get() - profile is " + profile);
        return profile;
    }
    ...
    public void saveProfileList(Context context) {
        ...
        editor.apply();
        Log.d("ProfileManager", "saveProfileList() - editor.apply() called");
    }
    ...
    private void loadVPNList(Context context) {
        ...
        Log.d("ProfileManager", "loadVPNList -> loaded list of vpns from sharedpreferences: " + vlist);

        if (vlist == null) {
            vlist = new HashSet<>();
        } else {
            vlist = new HashSet<>(vlist); // Create a copy of the set to avoid modifying the original
        }
        ...
        for (String vpnentry : vlist) {
            ...
            try {
                String fileName = vpnentry + ".vp";
                if (fileExists(context, fileName)) {
                    vpnfile = new ObjectInputStream(context.openFileInput(fileName));
                    ...
                    Log.d("ProfileManager", "loadVPNList -> did not find file " + vpnentry);
                    notFound.add(vpnentry);
                }
                ...
                if (vpnfile != null) {
                    try {
                        ...
                        Log.e(TAG, "Error closing VPN file", e);
                    }
                }
            }
        }

        for (String curNotFound : notFound) {
            removeProfile(context, curNotFound);
        }

        Log.d("ProfileManager", "loadVPNList -> loaded " + profiles.size() + " profiles: " + profiles.keySet());
    }


    public void removeProfile(Context context, VpnProfile profile) {
        removeProfile(context, profile.getUUID().toString());

        if (mLastConnectedVpn == profile)
            mLastConnectedVpn = null;
    }

    public void removeProfile(Context context, String vpnentry) {
        Log.d("ProfileManager", "removeProfile(" + vpnentry + ")");
    ...
    public void deleteAllProfiles(Context context) {
        Log.d("ProfileManager", "deleteAllProfiles");
        for (VpnProfile curProfile : profiles.values()) {
            removeProfile(context, curProfile);
        }
    }
}