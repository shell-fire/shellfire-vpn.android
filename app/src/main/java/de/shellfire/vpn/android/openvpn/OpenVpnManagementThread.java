...
public class OpenVpnManagementThread implements Runnable, OpenVPNManagement {
    ...
    private transient Connection mCurrentProxyConnection;    private final Runnable mResumeHoldRunnable = () -> {
        if (shouldBeRunning()) {
            releaseHoldCmd();
        }
    };
    ...
    public void run() {
        ...
        try {
            ...
            if (e.getMessage() != null && !e.getMessage().equals("socket closed") && !e.getMessage().equals("Connection reset by peer"))
                VpnStatus.logException(e);
        }
        synchronized (active) {
            active.remove(this);
    ...
    private void protectFileDescriptor(FileDescriptor fd) {
        try {
            ...
            int fdint = fd != null ? (Integer) getInt.invoke(fd) : 0;
